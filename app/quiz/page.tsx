...
export default async function QuizPage({ searchParams }: { searchParams: Promise<{ admin?: string; book?: string }> }) {
  ...
  const [chapters, collections, navData, progress, recentBook] = await Promise.all([
  ...
  return (
    <AppShell navData={navData} isAuthenticated={!!user}>
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-8 pt-14 md:pt-8 pb-24">
        <QuizArena chapters={chapters} collections={collections} isAuthenticated={!!user} allowAdminBrowse={!!user && query.admin === '1'} progress={progress} initialBook={query.book ?? recentBook ?? chapters[0]?.bookSlug} />
      </div>
    </AppShell>
  )
}
