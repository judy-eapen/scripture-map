...
export default async function MarkResourcesPage({ searchParams }: { searchParams: Promise<{ chapter?: string; slide?: string; view?: string }> }) {
  ...
  const lessonView = ['read', 'culture', 'connections', 'places', 'recall'].includes(params.view ?? '') ? params.view! : 'read'
      ...
      <main className="flex-1 min-w-0 overflow-y-auto p-4 md:p-7">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(201,168,76,0.16)', background: 'var(--navy-900)' }}>
            ...
            {selected.kind === 'images' && slideImage ? <div className="flex items-center justify-center p-2 md:p-4" style={{ background: '#090e1c' }}><Image src={slideImage} alt={`Mark ${selected.chapter} teacher slide ${slide}`} width={1600} height={900} priority className="block w-full h-auto rounded-lg" /></div> : lesson && <div className="p-4 md:p-8" style={{ background: '#090e1c' }}><article className="max-w-3xl mx-auto rounded-2xl p-5 md:p-8" style={{ background: 'linear-gradient(145deg, rgba(201,168,76,0.08), rgba(255,255,255,0.025))', border: '1px solid rgba(201,168,76,0.2)' }}><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs uppercase tracking-widest" style={{ color: 'var(--gold-400)' }}>{lesson.eyebrow}</p><strong className="text-sm" style={{ color: 'var(--ivory-200)' }}>{lesson.passage}</strong></div><h2 className="text-3xl mt-3" style={{ fontFamily: 'var(--font-playfair)', color: 'var(--ivory-100)' }}>{lesson.title}</h2>{lesson.image && <figure className="mt-5 overflow-hidden rounded-xl" style={{ border: '1px solid rgba(255,255,255,0.09)' }}><Image src={lesson.image.src} alt={lesson.image.alt} width={1600} height={900} className="block w-full h-auto" /><figcaption className="px-3 py-2 text-[11px]" style={{ color: 'var(--muted-500)' }}>{lesson.image.caption}</figcaption></figure>}<p className="mt-3 leading-relaxed" style={{ color: 'var(--ivory-200)' }}>{lesson.summary}</p><div className="flex flex-wrap gap-2 mt-6">{['read', 'culture', 'connections', 'places', 'recall'].map(view => <Link key={view} href={`/resources/mark?chapter=${selected.chapter}&slide=${slide}&view=${view}`} className="rounded-full px-3 py-1.5 text-xs font-semibold capitalize" style={{ color: lessonView === view ? 'var(--navy-950)' : 'var(--muted-400)', background: lessonView === view ? 'var(--gold-400)' : 'rgba(255,255,255,0.05)' }}>{view}</Link>)}</div>{lessonView === 'read' && <div className="mt-6 space-y-3"><ul className="space-y-3">{lesson.details.map(detail => <li key={detail} className="flex gap-3 text-sm leading-relaxed" style={{ color: 'var(--muted-300)' }}><span style={{ color: 'var(--gold-400)' }}>◆</span>{detail}</li>)}</ul>{lesson.carefulNote && <p className="rounded-xl p-4 text-sm leading-relaxed" style={{ background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.2)', color: '#bfdbfe' }}><strong>Read carefully:</strong> {lesson.carefulNote}</p>}</div>}{lessonView === 'culture' && <div className="mt-6 space-y-3">{lesson.culture?.length ? lesson.culture.map(item => <div key={item.title} className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.035)' }}><h3 className="text-sm font-semibold" style={{ color: 'var(--ivory-100)' }}>{item.title}</h3><p className="text-sm mt-2 leading-relaxed" style={{ color: 'var(--muted-400)' }}>{item.body}</p>{item.source && <a href={item.source.href} target="_blank" rel="noreferrer" className="inline-block text-xs mt-2 underline" style={{ color: 'var(--gold-300)' }}>Source: {item.source.label} ↗</a>}</div>) : <p className="text-sm" style={{ color: 'var(--muted-400)' }}>No additional cultural note for this overview.</p>}</div>}{lessonView === 'connections' && <div className="mt-6 space-y-3">{lesson.connections.map(connection => <div key={`${connection.label}-${connection.references}`} className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.08)' }}><p className="text-xs font-semibold" style={{ color: 'var(--gold-300)' }}>{connection.label} · {connection.references}</p><p className="text-sm mt-2 leading-relaxed" style={{ color: 'var(--muted-400)' }}>{connection.explanation}</p></div>)}</div>}{lessonView === 'places' && <MarkLessonPlacesPanel chapter={selected.chapter} />}{lessonView === 'recall' && <ol className="mt-6 space-y-3">{lesson.recall.map((question, index) => <li key={question} className="rounded-xl p-4 text-sm" style={{ background: 'rgba(255,255,255,0.035)', color: 'var(--ivory-200)' }}><span className="mr-2" style={{ color: 'var(--gold-400)' }}>{index + 1}.</span>{question}</li>)}</ol>}</article></div>}
