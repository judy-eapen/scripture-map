...
function BookCard({ book, isAuthenticated, progress }: { book: LibraryBook; isAuthenticated: boolean; progress?: LibraryProgress[string] }) {
  ...
  return <article className="min-w-0 rounded-3xl p-5 sm:p-6 flex flex-col self-start" style={{ ...panel, background: 'rgba(255,255,255,0.035)' }}>
    <div className="min-w-0 flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="text-[11px] uppercase tracking-widest" style={{ color: 'var(--muted-500)' }}>{guide.testament}</p><h2 className="text-2xl mt-1" style={{ fontFamily: 'var(--font-playfair)', color: 'var(--ivory-100)' }}>{guide.title}</h2></div>{guide.translationLabel && <span className="max-w-full text-[10px] rounded-full px-2.5 py-1 text-right" style={{ background: 'rgba(201,168,76,0.09)', color: 'var(--muted-400)' }}>{guide.translationLabel}</span>}</div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2"><Link href={`/study/${book.slug}/${nextChapter?.number ?? 1}`} className="rounded-xl px-4 py-2.5 text-center text-sm font-semibold" style={{ background: 'var(--gold-400)', color: 'var(--navy-950)' }}>{isAuthenticated && readIds.size ? `Continue at ${guide.shortTitle} ${nextChapter?.number ?? 1}` : 'Read'}</Link><Link href={`/quiz?book=${book.slug}`} className="rounded-xl px-4 py-2.5 text-center text-sm font-semibold" style={{ border: '1px solid rgba(201,168,76,0.32)', color: 'var(--gold-300)' }}>Quiz</Link></div>
...
export default function WelcomePageClient({ books, isAuthenticated, progress }: Props) {
  const tools = [
    { key:'kings', label:'Kings', links:[{ href:'/timeline', label:'Kingdom Timeline' }, { href:'/genealogy', label:'Dynasty Web' }] },
    { key:'mark', label:'Mark', links:[{ href:'/resources/mark', label:'Teacher slides' }] },
    { key:'all', label:'All books', links:[{ href:'/quiz', label:'Quiz' }, { href:'/flashcards', label:'Flashcards' }, { href:'/notes', label:'My Notes' }] },
  ]
  const steps = [
  ...
  return <main className="min-h-screen overflow-x-hidden px-5 py-12 sm:px-8 md:py-16" style={{ background: 'var(--navy-950)' }}><div className="min-w-0 max-w-6xl mx-auto">
    ...
    <section className="min-w-0 mt-12 rounded-3xl p-5 sm:p-6" style={panel} aria-labelledby="study-path"><h2 id="study-path" className="text-xs uppercase tracking-widest text-center mb-5" style={{ color: 'var(--gold-300)' }}>How to study here</h2><div className="min-w-0 grid md:grid-cols-3 gap-5">{steps.map(step => <div key={step.title} className="min-w-0 flex md:block gap-3 text-left md:text-center"><step.icon className="shrink-0 md:mx-auto mb-2" size={21} style={{ color: 'var(--gold-300)' }} /><div className="min-w-0"><h3 className="text-sm font-semibold" style={{ color: 'var(--ivory-100)' }}>{step.title}</h3><p className="text-xs leading-relaxed mt-1" style={{ color: 'var(--muted-400)' }}>{step.text}</p></div></div>)}</div></section>
    ...
    <section className="mt-12 rounded-3xl p-5 sm:p-6" style={panel}><p className="text-xs uppercase tracking-widest mb-4" style={{ color: 'var(--muted-500)' }}>Study tools</p><div className="flex flex-wrap gap-x-8 gap-y-4">{tools.map(({ key, label, links }) => <div key={key} className="flex flex-wrap items-center gap-2"><span className="text-sm font-semibold" style={{ color: 'var(--ivory-100)' }}>{label}:</span>{links.map((tool, i) => <span key={tool.href} className="flex items-center gap-2"><Link href={tool.href} className="text-sm underline underline-offset-4" style={{ color: 'var(--gold-300)' }}>{tool.label}</Link>{i < links.length - 1 && <span style={{ color: 'var(--muted-500)' }}>·</span>}</span>)}</div>)}</div></section>
  </div></main>
}
